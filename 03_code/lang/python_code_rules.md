...
Cursor エディタでの Python 開発におけるコーディングルール。可読性・保守性・一貫性を重視し、チーム全体で統一した品質を実現します。
...
- インデント: スペース 4 つ
- 最大行長: 79 文字以内（80 ～ 100 文字も許容）
- 改行コード: LF
- ファイル末尾に改行を挿入
- 空行: 関数／クラス定義前に 2 行、メソッド内は 1 行
- 自動整形: `black` を導入しコードをフォーマット
- Import 順序: 標準 → サードパーティ → ローカル、各グループ間に空行（`isort`推奨）
...
- モジュール／クラス／関数には必ず Docstring を記述
- 形式: Google スタイルまたは NumPy スタイル（プロジェクトで統一）
...
- [ ] PEP8 に準拠しているか（`flake8`エラーなし）
- [ ] `black`でフォーマット済みか
- [ ] `isort`で Import 順序が正しいか
- [ ] 型ヒントが適切に記述されているか
- [ ] Docstring が記載されているか
- [ ] 例外処理が適切に実装されているか
- [ ] ログレベルが適切に使い分けられているか
